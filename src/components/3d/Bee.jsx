...
function Bee(props) {
  const group = useRef()
  const { nodes, materials, animations } = useGLTF('/bee_minecraft.glb')
  const { actions } = useAnimations(animations, group)

  // Play the animation when the component mounts
  useEffect(() => {
    if (actions) {
      // Play the first available animation or any named animation
      actions[Object.keys(actions)[0]].play()
    }
  }, [actions]) // This effect will run whenever `actions` is updated
...
useGLTF.preload('/bee_minecraft.glb')
